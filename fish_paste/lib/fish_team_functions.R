...
Calc_Biomassgm2<-function(x){
	...
	Area<-ifelse(x$METHOD =="nSPC", pi*(7.5^2), ifelse(x$SIZE_ < 20, 50, 100))
...
{	
	...
	for (i in 1:length(tmp)) {
		s<-tmp[i]
		if (nchar(s)<8) {   # only change values where name length is too short ()
			ss<-strsplit(as.character(tmp[i]),"-")
			s1<-ss[[1]][1]
			s2<-ss[[1]][2]
			if (length(x=grep("[A-Z]",unlist(strsplit(toupper(s2),""))))==0) 
			{
				tmp[i]<-paste(s1, formatC(as.numeric(s2), width=4, flag="0"), sep="-")
			}
		}
	}
